 ...
 * @author Mariem
 */
public class ModifCovoiturageController implements Initializable {
	...
	public void getUser(Covoiturage C){
		...
		Covoiturage R=G.RechercheCovoiturage(C.getId());
	...
	private void ModifierCovoiturage(ActionEvent event) throws IOException {
       ...
       G.AjouterCovoiturage(C);
		...
		String destination=adresseDestinationTxt.getText();
		Covoiturage Cov=(Covoiturage) btnModif.getUserData();
		Covoiturage C1=new Covoiturage(ladate,nbr_places,heure_dep,depart,destination,cin);
       GestionCovoiturage G1=new GestionCovoiturage();
	   G1.ModifierCovoiturage(C1,Cov.getId());
	   */
	...
	private void SupprimerCovoiturage(ActionEvent event) throws IOException {
        ...
        if (action.get()==ButtonType.OK){
         ...
         Covoiturage Cov=(Covoiturage) btnSupp.getUserData();
